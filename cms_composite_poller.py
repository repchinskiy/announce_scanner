#!/usr/bin/env python3
"""
cms_composite_poller.py — alternative CMS (composite) endpoint poller for announce_scanner

Sixth channel (optional, comparison only). Polls the alternative Binance CMS
endpoint `/bapi/composite/v1/public/cms/article/catalog/list/query` used by
rickstaa/crypto-listings-sniper, to compare its CloudFront cache behaviour
with the primary CMS endpoint (`/bapi/apex/...`) used by cms_apex_poller.py.

Comparison summary (verified 2026-07-21):
  - Both paths are CloudFront-cached with identical TTL (~60-75s)
  - Both return ~110ms Hit / ~800ms Miss latency
  - apex path returns `releaseDate` (epoch ms) — needed for latency calc
  - composite path does NOT return `releaseDate` (only id/code/title)
  - JSON shape differs: composite has `data.articles[]` (flat);
    apex has `data.catalogs[].articles[]` (nested)

This channel exists to empirically confirm the cache behaviour is identical
and to detect any divergence in production (e.g. if Binance changes caching
on one path but not the other).

Multi-catalog support: monitors the same set of catalogIds as cms_apex_poller.py
via the shared `ANN_CATALOG_IDS` env var (default: 48). Set to `0` to monitor
ALL known categories. This keeps behavior consistent across CMS channels.

Uses a randomized pageSize (10..49) per request as a weak cache-buster
(same technique as rickstaa). Conservative 60s interval matching rickstaa's
safe rate (their code comment: "Don't set above 0.016666667 Hz or binance
will (temporary) ban your IP").

Usage:
    python cms_composite_poller.py             # continuous polling
    python cms_composite_poller.py --oneshot   # single check, print + exit
    python cms_composite_poller.py --reset     # reset stored state

Config (env, optional):
    ANN_CATALOG_IDS              shared with cms_apex_poller.py (default: 48)
                                 Set to 0 to monitor ALL known categories.
    COMPOSITE_POLL_INTERVAL_S    base interval (default 60.0)
"""
...
# Config
# ---------------------------------------------------------------------------
CMS_BASE = "https://www.binance.com/bapi/composite/v1/public/cms/article/catalog/list/query"
...
POLL_INTERVAL_S = float(os.environ.get("COMPOSITE_POLL_INTERVAL_S", "60.0"))

HTTP_TIMEOUT_S = 5.0

PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
STATE_DIR = os.path.join(PROJECT_ROOT, "state")
STATE_FILE = os.path.join(STATE_DIR, "composite_state.json")


# ---------------------------------------------------------------------------
# Catalog IDs (shared logic with cms_apex_poller.py)
# ---------------------------------------------------------------------------
# Known categories (same as cms_apex_poller.py):
...
# "0" means "monitor ALL known categories" — expand it (the composite endpoint
# requires a concrete catalogId per request, same as the apex endpoint).
if CATALOG_IDS == [0]:
...
def extract_articles(data: dict[str, Any]) -> list[dict[str, Any]]:
    """Composite path returns articles flat under data.articles (no catalogs wrapper)."""
    try:
        d = data.get("data", {}) or {}
...
async def poll_one(session: aiohttp.ClientSession,
    ...
    if status_code == 429:
        log.warning(f"[CMS_COMPOSITE] 429 rate-limited  cat={catalog_id}  key={key_label}")
        notifier.reconnect("CMS_COMPOSITE", f"429 rate-limited (cat={catalog_id}, key={key_label})")
        stats.setdefault("err_count", 0)
        stats["err_count"] += 1
    elif status_code != 200:
        log.warning(f"[CMS_COMPOSITE] HTTP {status_code}  cat={catalog_id}  key={key_label}  cache={cache_status}")
        notifier.reconnect("CMS_COMPOSITE", f"HTTP {status_code} (cat={catalog_id}, key={key_label})")
        stats.setdefault("err_count", 0)
        stats["err_count"] += 1
    ...
    if art_id > known_max_id and emit:
        ...
        log.info(
            f"🚀 NEW [CMS_COMPOSITE]  cat={catalog_id}  id={art_id}  code={code}  "
...
async def run_continuous() -> None:
    ...
    log.info(f"[CMS_COMPOSITE] announce_scanner  catalogs=[{cats_str}]  "
    ...
    async with create_http_client(timeout=HTTP_TIMEOUT_S) as session:
        ...
        log.info(f"[CMS_COMPOSITE] ready  {ready_str}")
        ...
        while True:
            ...
            if cycle % int(os.environ.get("COMPOSITE_STATS_EVERY_CYCLES", "10")) == 0:
                total = stats["total"]
                cache = stats["cache"]
                parts = []
                ...
                ready_str = " ".join(f"cat{c}={known[c]}" for c in CATALOG_IDS)
                log.info(
                    f"[CMS_COMPOSITE] STATS  cycle={cycle}  {ready_str}  "
                ...
                if delta_err > 0 and delta_reqs > 0 and (delta_err / delta_reqs) > 0.05:
                    ...
                    log.warning(f"[CMS_COMPOSITE] {delta_err} new errors ({rate:.0f}% of requests, total: {current_err})")
                    notifier.reconnect("CMS_COMPOSITE", f"{delta_err} new errors ({rate:.0f}% rate), total {current_err}")
                elif delta_err > 0 and delta_reqs == 0:
                    # All requests failed — 100% error rate.
                    log.warning(f"[CMS_COMPOSITE] {delta_err} new errors (100% of requests, total: {current_err})")
                    notifier.reconnect("CMS_COMPOSITE", f"{delta_err} new errors (100% rate), total {current_err}")
...
# ---------------------------------------------------------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description="Composite CMS poller for announce_scanner")
    ...
    try:
        ...
        log.info("[CMS_COMPOSITE] stopped")
        notifier.shutdown("CMS_COMPOSITE", "interrupted by user")
        try:
